...
To start the tracker, ensure ~tracker-mode.el~ is in your ~load-path~, and then type ~M-: (require 'tracker-mode) RET~ to load it. To start a new track, either open a file with a ~.trk~ or ~.track~ file extension, or type ~M-x tracker~ to automatically create a new buffer and start the mode.
